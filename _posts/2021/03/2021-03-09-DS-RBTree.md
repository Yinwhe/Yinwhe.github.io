...
1. 对任何一个节点$x$，有$sizeof(x)\ge2^{bh(x)}-1$；其中$bh(x)$表示子树$x$包含的黑色节点数

    使用归纳法证明：当 $h(x)=0$ 的时候，即$x=NIL$，结论显然成立

    假设$h(x)\le k$ 时都成立，考虑$k+1$的情况。对于$x$的子节点，有$bh(child)=bh(x)\ or\ bh(x)-1$，且有$sizeof(x)\ge 2^{bh(x)}-1\ge 2^{bh(x)-1}-1$
