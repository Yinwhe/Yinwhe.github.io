...
每次按下一个字符时，都会导致最右边的转轮`+1`，且后续的计算是在其`+1`后进行的，比如当前的外部密码是`AAA`，那么按下`A`的时候，最右轮`+1`即`AAB`，然后`A`的加密是在`AAB`的密码下进行的。

另外轮是26进制的，故也存在进1的问题，但是每个Rotor进位的位置是不同的：
...
此时再旋转I的话，I本来是不应该带动II转的(因为当前I不在Q这个位置), 但是II还会再转(double stepping)，
同时II带动III旋转: III=2=B, II=6=F, I=19=S

归纳起来讲, II有两种情况会转动:
(1) I从Q转到R
(2) II当前在E位置, I不管在什么位置
double stepping是由Enigma的机械结构决定的，该现象只会出现在中间那个齿轮上。
```